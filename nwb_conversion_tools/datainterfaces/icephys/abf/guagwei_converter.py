...
def run_conversion(source_dir, output_file=None, metadata_file=None, icephys_experiment_type=None):
    ...
    class GuagweiConverter(NWBConverter):
        data_interface_classes = {f"AbfNeoDataInterface_{i}": AbfNeoDataInterface for i, _ in enumerate(files_list)}

    # Instantiate Converter
    source_data = {f"AbfNeoDataInterface_{i}": dict(file_path=f) for i, f in enumerate(files_list)}
