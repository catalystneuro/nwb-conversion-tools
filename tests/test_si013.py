...
from nwb_conversion_tools.utils import export_to_nwb, SI013NwbEphysWriter
...
class TestExtractors(unittest.TestCase):
    ...
    def test_write_recording(self):
        ...
        export_to_nwb(objects_to_write=self.RX, nwb_file_path=path, overwrite=True)
        RX_nwb = se.NwbRecordingExtractor(path)
        check_recording_return_types(RX_nwb)
        check_recordings_equal(self.RX, RX_nwb)
        check_dumping(RX_nwb)
        ...
        export_to_nwb(
            objects_to_write=self.RX, nwb_file_path=path, overwrite=True
    ...
    def test_write_sorting(self):
        ...
        export_to_nwb(objects_to_write=self.RX, nwb_file_path=path, overwrite=True)
        export_to_nwb(objects_to_write=self.SX, nwb_file_path=path, overwrite=False)
        ...
        export_to_nwb(objects_to_write=self.SX, nwb_file_path=path, skip_properties=["stability"], overwrite=True)
        SX_nwb = se.NwbSortingExtractor(path, sampling_frequency=sf)
    ...
    def test_nwb_metadata(self):
        ...
        export_to_nwb(objects_to_write=self.RX, metadata=metadata2, nwb_file_path=path, overwrite=True)
        ...
        export_to_nwb(objects_to_write=self.RX, metadata=metadata3, nwb_file_path=path, overwrite=True)
...
class TestWriteElectrodes(unittest.TestCase):
    ...
    def test_append_same_properties(self):
        export_to_nwb(objects_to_write=self.RX, nwbfile=self.nwbfile1, metadata=self.metadata_list[0], es_key="es1")
        export_to_nwb(objects_to_write=self.RX2, nwbfile=self.nwbfile1, metadata=self.metadata_list[1], es_key="es2")
